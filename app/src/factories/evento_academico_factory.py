...
class EventoAcademicoFactory(EventoFactory):
    ...
    def crear_evento(self, tipo, nombre, fecha, ubicacion):
        if tipo not in ["conferencia", "seminario"]:
            raise ValueError("Tipo de evento académico no soportado")
        
        return EventoAcademicoDTO(nombre, fecha, ubicacion, tipo)

    def obtener_eventos(self):
        """Aquí deberías conectar con SQL o MongoDB para obtener los eventos académicos"""
        return [
