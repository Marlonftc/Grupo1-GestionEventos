# Importamos la clase base EventoDTO para heredar sus atributos y métodos
from .evento_dto import EventoDTO

class ConferenciaDTO(EventoDTO):
    """
    Clase que representa un evento de tipo 'Conferencia'.
    Hereda de EventoDTO e inicializa la categoría como 'conferencia'.
    """

    def __init__(self, id_evento, nombre, fecha, ubicacion):
        """
        Constructor de la clase ConferenciaDTO.

        :param id_evento: Identificador único del evento.
        :param nombre: Nombre de la conferencia.
        :param fecha: Fecha del evento en formato datetime o string compatible.
        :param ubicacion: Ubicación donde se llevará a cabo la conferencia.
        """
        # Llamamos al constructor de la clase base (EventoDTO) y establecemos la categoría como 'conferencia'
        super().__init__(id_evento, nombre, fecha, ubicacion, "conferencia")
